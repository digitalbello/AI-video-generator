...
# Store custom prompts in session state so they persist
if "custom_prompts" not in st.session_state:
    st.session_state.custom_prompts = []

# Show prompt preview button BEFORE the generate button
show_preview = st.button("🔍 Preview Image Prompts", type="secondary")

if show_preview and script.strip():
    with st.spinner("🧠 Analyzing script..."):
        preview_scenes, _ = parse_script(script)
        if preview_scenes:
            st.session_state.custom_prompts = render_prompt_preview(preview_scenes)
            st.session_state.preview_scenes = preview_scenes
        else:
            st.error("Could not parse script. Please check the format.")
        ...
        try:
            ...
            if image_source == "🤖 AI Generate Images from Script":
                with st.spinner("🎨 AI is generating cinematic images..."):
                    # Use custom prompts from preview if available, otherwise None
                    custom_prompts = st.session_state.get("custom_prompts", None)
                    ai_images = generate_scene_images(scenes, custom_prompts)
